    ...
    while(count < 20)
    {
        ...
        for(int k = 0; k<4; ++k)
        {
            ...
            for(int i = 0; i < Ns*Ns*Nb*Nb; ++i){
                G1x_tmp[i] = complex<double>(G1x[i]) + K1x[i]*dt;
            }
            ...
            U_AS = fermi(t_RK,AS_midpoint,AS_rate)*U;
            if(EHM == true){
                for(int i = 0; i < Ns-1; ++i){
                    //V[i]=(U_AS*.5/(i+1));
                    V[i]=(U_AS*exp(-decay_rate*(i+1)));
                }
            }
            h_HFx_quench(h1x,G1x_tmp,G1x_tmp,Ns,Nb,U_AS,t1,t2,0,epsilon,V,0);
#if NOTHREADS
            hG_commutator(G1x_tmp,h1x,h1xG1x_comm);
            if(U!=0 && HF == false){
                collision_int(Ix,G2xyxy_tmp,G2xxxx_tmp,Ns,Nb,U_AS,V);
                ...
                Psi_xxxx(psi_xxxx,G1x_tmp,Ns,Nb,U_AS,V);
                Psi_xyxy(psi_xyxy,G1x_tmp,Ns,Nb,U_AS,V);

                Pi_xyxy(pi_xxxx,G1x_tmp,G2xyxy_tmp,G2xxxx_tmp,U_AS,Ns,Nb,V);
                Pi_xyxy(pi_xyxy,G1x_tmp,G2xxxx_tmp,G2xyxy_tmp,U_AS,Ns,Nb,V);
            ...
            thread th_collision_int(collision_int,ref(Ix),ref(G2xyxy_tmp),ref(G2xxxx_tmp),Ns,Nb,U_AS,ref(V));
            ...
            thread th_psi_xxxx(Psi_xxxx,ref(psi_xxxx),ref(G1x_tmp),Ns,Nb,U_AS,ref(V));
            thread th_psi_xyxy(Psi_xyxy,ref(psi_xyxy),ref(G1x_tmp),Ns,Nb,U_AS,ref(V));

            thread th_pi_xxxx(Pi_xyxy,ref(pi_xxxx),ref(G1x_tmp),ref(G2xyxy_tmp),ref(G2xxxx_tmp),U_AS,Ns,Nb,ref(V));
            thread th_pi_xyxy(Pi_xyxy,ref(pi_xyxy),ref(G1x_tmp),ref(G2xxxx_tmp),ref(G2xyxy_tmp),U_AS,Ns,Nb,ref(V));
