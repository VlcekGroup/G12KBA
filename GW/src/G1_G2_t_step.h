...
void G2_step(vector<complex<double>> &h2G2_comm, vector<complex<double>> &psi, vector<complex<double>> &pi, vector<complex<double>> &K,vector<complex<double>> &RK_result, double w,int Ns,int Nb,double U)
{
                        ...
                        for(int g = 0; g < Nb; ++g){
                            for(int l = 0; l< Ns; ++l){
                                for(int k = 0; k < Ns; ++k){
                                    ...
                                    //int idx2 = IDX_8D(j,i,a,b,l,k,g,d); //This is what is in G1-G2 paper but seems like a mistake                                    
                                    K[idx1] = -im*(h2G2_comm[idx1] +  U*psi[idx1] + pi[idx1] - conj(pi[idx2]));
                                    RK_result[idx1] += w*K[idx1];
                                }
							}
						}
