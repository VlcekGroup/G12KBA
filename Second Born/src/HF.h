        ...
        for(int i = 0; i < Ns; ++i)
        {
                for(int a = 0; a < Nb; ++ a)
                {
                        ...
                        for(int g = 0; g < Nb; ++g)
                        {
                                ...
                                //h[(i +a*Ns)*Nb*Ns + (j+b*Ns)] += im*U*kron_delta(i,j)*kron_delta_bar(a,b)*G1[(i+b*Ns)*Ns*Nb + i + a*Ns];
                }
        }
	...
	complex<double> im = {0,1};
        for(int i = 0; i < Ns; ++i)
        {
                for(int a = 0; a < Nb; ++ a)
                {
                        ...
                        if(a < Nb-1)
                        {
                                h[(i+a*Ns)*Ns*Nb + (i + (a+1)*Ns)] = P*exp(-pow(t/sig,2))*sin(w*t) - t2;
                                h[(i+(a+1)*Ns)*Ns*Nb + (i + a*Ns)] = P*exp(-pow(t/sig,2))*sin(w*t) - t2;
                        }
                        for(int g = 0; g < Nb; ++g)
                        {
                                if(g!=a)
